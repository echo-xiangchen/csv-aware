...
public class GenCrossCompCallFlow {
	public static void main(String[] args) {
		if (args.length < 1 ) {
			System.out.println("Usage: "
					+ "Input: a .csv file that contains the result of cross component call flow.\n"
					+ "(a:exitFunc)-[:call]->(b:entryFunc)-[:compCall]->(c:exitFunc) WHERE a.comp <> b.comp AND b.comp = c.comp\n"
					+ "Output: 1. a .csv file of CompCall relationship.\n"
			...
			try {
				...
				// create writers
				writer = new BufferedWriter(new FileWriter("crossCompCallFlow.csv"));
		    	...
		    	while ((line = reader.readLine()) != null) {
					...
					String crossCompCallFlow = startID + "\t" + endID + "\t" 
									+ "crossCompCallFlow" + "\t" + condition + "\n";
					
					// write the callClosure fact to "callClosure.csv"
					writer.write(crossCompCallFlow);
				}
					
				writer.close();
			    System.out.println("Finish writing crossCompCallFlow.csv");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
