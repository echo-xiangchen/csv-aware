...
public class GenInitAssign {
	public static void main(String[] args) {
		if (args.length < 1 ) {
			System.out.println("Usage: "
					+ "Input: a .csv file that contains the result of initial assignment of BA.\n"
			...
			try {
				...
				// create writers
				writer = new BufferedWriter(new FileWriter("initAssign.csv"));
		    	...
		    	while ((line = reader.readLine()) != null) {
					...
					// generate condition for current line: the initAssign relationship
					String condition = "";
					for (String pc : PCset) {
					...
					String initAssign = startID + "\t" + endID + "\t" 
									+ "initAssign" + "\t" + condition + "\n";
					
					// write the callClosure fact to "callClosure.csv"
					writer.write(initAssign);
				}
					
				writer.close();
			    System.out.println("Finish writing initAssign.csv");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
