...
public class GenIntraBA {
	public static void main(String[] args) {
		if (args.length < 1 ) {
			System.out.println("Usage: "
					+ "Input: a .csv file that contains the result of intra-comp of BA.\n"
					+ "Output: 1. a .csv file of intraBA relationship.\n"
			...
			try {
				...
				// create writers
				writer = new BufferedWriter(new FileWriter("intraBA.csv"));
		    	...
		    	while ((line = reader.readLine()) != null) {
					...
					// generate condition for current line: the intraBA relationship
					String condition = "";
					for (String pc : PCset) {
					...
					String intraBA = startID + "\t" + endID + "\t" 
									+ "intraBA" + "\t" + condition + "\n";
					
					// write the callClosure fact to "callClosure.csv"
					writer.write(intraBA);
				}
					
				writer.close();
			    System.out.println("Finish writing intraBA.csv");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
